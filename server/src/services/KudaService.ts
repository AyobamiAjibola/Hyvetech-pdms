...
class KudaService implements BankService {
    ...
    this.network.interceptors.response.use(
      ...
      error => {
        ...
        if (error.response && error.response.data) {
          this.logger.error(error?.response?.data, error?.request?.path, error?.request?.method, error?.request?.body);
          return Promise.reject(error);
        }
        this.logger.error(error?.message, error?.request?.path, error?.request?.method, error?.request?.body);
        return Promise.reject(error);
      },
    );
  }
  ...
  @TryCatch
  async getToken() {
    const response = await axios.post(`${settings.kuda.host}/Account/GetToken`, {
