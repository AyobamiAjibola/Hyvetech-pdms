...
export const handleDisableTimeslots = authenticateRouteWrapper(async (req: Request, res: Response) => {
  const result = await TimeSlotController.disableTimeslot(req);
  res.status(result.code).json(result);
});

export const handleGetTimeslots = authenticateRouteWrapper(async (req: Request, res: Response) => {
