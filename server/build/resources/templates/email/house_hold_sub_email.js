...
function house_hold_sub_email(config) {
    const getLocationText = () => {
        let text = '';
        if (config.planCategory === constants_1.MOBILE_CATEGORY)
            text = 'your location (no need to stress, tell us where and when, and we will come to you).';
        if (config.planCategory === constants_1.DRIVE_IN_CATEGORY)
    ...
    ${config.planCategory} inspections at ${getLocationText()}`;
        if (config.planCategory === constants_1.HYBRID_CATEGORY) {
...
exports.default = house_hold_sub_email;
/*
