...
export const createJobCheckListAction = asyncThunkWrapper<
  ApiResponseSuccess<any>,
  ICreateJobCheckList
>(CREATE_JOB_CHECK_LIST, async ({ jobId, checkList }) => {
  ...
  for (const image of images)
    files.push(
      await fetch(image.url)
        .then((r) => r.blob())
        .then(
          (blobFile) => new File([blobFile], image.title, { type: "image/png" })
        )
    );
